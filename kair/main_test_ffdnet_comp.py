...
from kair.utils import utils_logger, utils_image as util
...
def main():
    ...
    testset_name = 'ok'               # test set,  'bsd68' | 'cbsd68' | 'set12'
    ...
    indir = '/home/alex/Documents/dbs/fs_zip'
    outdir = indir + '_denoising'

    for gif_id in os.listdir(os.path.join(indir)):
        asset_dir = os.path.join(indir, gif_id)
        org_dir = os.path.join(asset_dir, 'org')
        out_org = org_dir.replace(indir, outdir)
        os.makedirs(out_org, exist_ok=True)
        img_names = sorted(os.listdir(org_dir))
        ...
        for idx, img_name in enumerate(img_names):
            img = os.path.join(org_dir, img_name)
        # logger.info('{:->4d}--> {:>10s}'.format(idx+1, img_name+ext))
            ...
            util.imsave(img_E, os.path.join(out_org, img_name))
